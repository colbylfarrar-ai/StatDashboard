# Lineup Projection + 4-Factor Optimizer — Design (B + C)
...
- **B — depth-chart / minutes projection.** Given a minutes allocation across the
  ...
  tracked team.
- **C — 4-factor lineup optimizer.** Search minute allocations to *maximize* projected
  win value (Dean Oliver four-factor weighting), subject to real coaching constraints
  (foul trouble, star stagger, min/max minutes). The crown jewel.
...
1. **Blend projected + observed.** For each stat, the team's projected four factors are
   the **minute-weighted** mix of each player's projected intrinsic rate
   (`projection`), because rates travel and minutes are the environment weight the base
   layer deliberately left out. Where a 5-man unit has enough observed possessions,
   fold `custom_unit`/`unit_ratings` Net in by credibility weight so real chemistry
   isn't thrown away.
2. **Four factors → efficiency.** Convert projected offensive & defensive four factors
   to projected ORtg/DRtg via the standard four-factor→efficiency relationship, then
   Net = ORtg − DRtg.
3. **Baseline.** Express Net and each factor as a delta vs the **average tracked team**.
4. Return `{minutes, factors:{off,def}, ortg, drtg, net, net_vs_baseline, flags}`.
   Every output carries the thin/directional flag discipline of `rotation_plan`.

### C — optimize the allocation

`optimize_minutes(team_id, ..., objective="net" | "four_factor")`:
...
- **Objective.** `four_factor` = Dean Oliver weighted score
  (eFG .40, TOV .25, OREB .20, FTR .15, offense minus the mirror on defense);
  `net` = projected Net vs baseline. Default `four_factor` (your premise: factors are
  the stable, coachable levers).
...
- **Output.** Recommended minutes, the projected factor line, Net vs baseline, and a
  **diff vs the team's *observed* minutes** — "shift 6 min from X to Y → +N/100,
  +M eFG%." That diff is the sellable "extra wins" prescription.
...
observed (diff highlighted), projected four-factor line vs average-tracked baseline, the
star-stagger note, and the current-roster projection. Team-gated behind `MIN_TEAM_GAMES`
with an honest "need more tracked games" empty state.
...
1. `project_minutes` factors are minute-weighted (double a player's minutes → team
   rate moves toward his rate, monotone).
...
6. `four_factor` objective uses the .40/.25/.20/.15 weights.
7. Team below `MIN_TEAM_GAMES` returns `gated`, no crash.
8. `project_team_current` returns a valid predictor payload for the deep team.
