...
def list_games(q: str | None = None):
    ...
    if len(q) >= 2:
        ...
        # Default picker = ONLY current-season tracked games (resume/continue).
        # The full schedule (OSSAA import = 13k+) is reachable solely via ?q=
        # search, so the picker can never auto-load a list big enough to freeze.
        games = query(
