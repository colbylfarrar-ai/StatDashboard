...
def pooled_game_ids(season="Current") -> set[int]:
    ...
    candidate set for every LEAGUE-WIDE tracked aggregation. `season` defaults to
    the active season; `in_pool` is frozen at rollover, so passing an archived
    label returns exactly what was shared THAT season (no leak, no inversion)."""
