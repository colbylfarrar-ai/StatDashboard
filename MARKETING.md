...
| **Self-serve free-trial funnel** | Wire the existing `paid_until` trial into a no-card "30-day full season" button; OIDC sign-in → auto-grant → guided first game → day-21 "your season so far" email with their own data. | Medium | The plumbing exists; the conversion moment is showing a coach THEIR data, so push to first-game-tracked fast. |
...
1. **Days 1–7 — Lock assets, pick one cluster.** Choose ONE home state/conference to concentrate density. Record the flagship 3–5 min demo + 3 cutdowns; write the comparison/pricing pin; set up the X account and YouTube channel. Verify the self-serve 30-day trial (Stripe webhook → `paid_until`) end-to-end so the funnel runs unattended.
